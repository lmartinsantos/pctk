...
func (o *Object) WithScript(v VerbType, script *Script) *Object {
	o.scripts[v] = script
	return o
}
...
func (o *Object) BinaryEncode(w io.Writer) (n int, err error) {
	...
	nn, err := BinaryEncode(w, uint32(len(o.scripts)))
	n += nn
	if err != nil {
		return n, err
	}

	for verb, script := range o.scripts {
...
func (o *Object) BinaryDecode(r io.Reader) error {
	...
	o.scripts = make(map[VerbType]*Script)
	for i := uint32(0); i < count; i++ {
		script := new(Script)
		var verb byte
		...
		o.scripts[VerbType(verb)] = script
